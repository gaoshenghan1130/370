...
void printStates(stateType *);

static inline int convertNum(int32_t);

enum opcodeType { ADD = 0, NOR, LW, SW, BEQ, JALR, HALT, NOOP };
...
struct instructionFields decoder(int instruction) {
  ...
  struct instructionFields fields = {.opcode = opcode,
                                     .regA = regA,
                                     .regB = regB,
                                     .destReg = destReg,
...
int get_num_mem_accesses() { return num_mem_accesses; }

int exec(stateType *statePtr, struct instructionFields fields) {
  switch (fields.opcode) {
  case ADD:
    statePtr->reg[fields.destReg] =
        statePtr->reg[fields.regA] + statePtr->reg[fields.regB];
    statePtr->pc += 1;
    break;
  case NOR:
    statePtr->reg[fields.destReg] =
  ...
  case LW: {
    ...
    statePtr->reg[fields.regB] = cache_access(address, 0, 0);
    statePtr->pc += 1;
  } break;
  case SW: {
    int offset = convertNum(fields.offsetField);
    ...
    cache_access(address, 1, statePtr->reg[fields.regB]);
    statePtr->pc += 1;
  } break;
  case BEQ: {
    int offset = convertNum(fields.offsetField);
 ...
 */
void printStates(stateType *statePtr) {
