...
int main(int argc, char *argv[]) {
  ...
  while (opcode(state.MEMWB.instr) != HALT) {
    ...
    int previousDest =
        (memwbOp == LW) ? field1(state.MEMWB.instr) : field2(state.MEMWB.instr);
    ...
    marks the end of the cycle and updates the current state with the values
